/**
 * Centralized Server-Side Pricing Engine for Professional SaaS Dashboard + Hardware Kit
 */
...
/**
 * Calculates complete pricing breakdown for a chosen plan key
 * @param {string} planKey - 'MONTHLY' | 'QUARTERLY' | 'HALF_YEARLY' | 'YEARLY'
 * @returns {Object} Complete pricing calculation object
 */
export function calculateQrBannerPrice(planKey) {
  ...
  // Check FREE Delivery Rule for >= 6 months
  const isOrderFeeFree = plan.durationMonths >= 6;
  const orderDeliveryFee = isOrderFeeFree ? 0 : plan.deliveryFee;
