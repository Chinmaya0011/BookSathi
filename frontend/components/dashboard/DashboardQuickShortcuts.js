...
export default function DashboardQuickShortcuts() {
  const shortcuts = [
    ...
    },
    {
      ...
      subtitle: 'Customer booking portal',
        ...
        {shortcuts.map((sc) => {
          ...
          return (
            <Link
              key={sc.href}
              href={sc.href}
              className={`p-2.5 rounded-xl bg-slate-50/70 border border-slate-200/70 transition-all duration-200 group text-left active:scale-95 shadow-2xs ${sc.hoverBorder} ${sc.hoverBg}`}
            >
              <div
